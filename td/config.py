...
class TdConfiguration:
    ...
    def __init__(self, config_path: str = "config/config.ini") -> None:
        ...
        if "TD_API_CONFIG_PATH" not in environ:
            ...
            self._caller_directory = Path(Path(inspect.stack()[-1].filename).parents[0])
        ...
        self.symbols = SymbolsConfiguration(self._config_parser)
        self.data_paths = DataPathsConfiguration(self._config_parser)
...
class CredentialsConfiguration:
    ...
    def __init__(self, config_parser):
        ...
        if config_parser.has_section("accounts"):
            ...
            self.account_password = config_parser.get("credentials", "account_password")
            self.secretquestion0 = config_parser.get("credentials", "secretquestion0")
            self.secretanswer0 = config_parser.get("credentials", "secretanswer0")
            self.secretquestion1 = config_parser.get("credentials", "secretquestion1")
            self.secretanswer1 = config_parser.get("credentials", "secretanswer1")
            self.secretquestion2 = config_parser.get("credentials", "secretquestion2")
            self.secretanswer2 = config_parser.get("credentials", "secretanswer2")
            self.secretquestion3 = config_parser.get("credentials", "secretquestion3")
            self.secretanswer3 = config_parser.get("credentials", "secretanswer3")
...
class AccountsConfiguration:
    ...
    def __init__(self, config_parser):
        ...
        if config_parser.has_section("accounts"):
            self.config_present = True
            self.default_account = config_parser.get("accounts", "default_account")
            self.roth_ira = config_parser.get("accounts", "roth_ira")
            self.traditional_ira = config_parser.get("accounts", "traditional_ira")
            self.cash_account = config_parser.get("accounts", "cash_account")
            self.margin_account = config_parser.get("accounts", "margin_account")
...
class LoggingConfiguration:
    ...
    ----------
    log_root_path : str
        The root directory path for the logs.
    use_bulk_app_name_logging : bool
        Whether to use a central app_name log file in addition to module level logging.
    ...
    def __init__(self, config_parser):
        ...
        if config_parser.has_section("logging"):
            self.config_present = True
            self.log_root_path = config_parser.get("logging", "log_root_path")
            self.use_bulk_app_name_logging = bool(
                config_parser.get("logging", "use_bulk_app_name_logging")
            )


class SymbolsConfiguration:
    """
    Handles symbols configuration data.
    ...
    def __init__(self, config_parser):
        """
        Initializes the SymbolsConfiguration object.
...
class DataPathsConfiguration:
    ...
    def __init__(self, config_parser):
        """
        Initializes the DataPathsConfiguration object.
